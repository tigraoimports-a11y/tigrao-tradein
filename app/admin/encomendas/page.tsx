...
export default function EncomendasPage() {
  ...
  const fetchData = useCallback(async () => {
    setLoading(true);
    try {
      const res = await fetch("/api/encomendas", { headers: { "x-admin-password": password } });
    ...
    <div className="space-y-6">
      ...
      {tab === "nova" ? (
        <div className="bg-white border border-[#D2D2D7] rounded-2xl p-6 shadow-sm space-y-6">
          <h2 className="text-lg font-bold text-[#1D1D1F]">Nova Encomenda</h2>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                  ...
                  <tr key={enc.id} className="border-b border-[#F5F5F7] hover:bg-[#F5F5F7] transition-colors">
                    ...
                    <td className="px-4 py-3">
                      <button onClick={async () => {
                        ...
                        await fetch("/api/encomendas", { method: "DELETE", headers: { "Content-Type": "application/json", "x-admin-password": password }, body: JSON.stringify({ id: enc.id }) });
