...
export async function GET(request: Request) {
  ...
  const url = `https://api.z-api.io/instances/${instanceId}/token/${token}/chats?page=1&pageSize=100`;
  ...
  try {
    ...
    if (!res.ok) {
      return NextResponse.json(
        { error: "Z-API retornou erro", status: res.status, details: data },
        { status: 502 }
      );
    }
    ...
    return NextResponse.json({ groups, totalChats: chats.length });
  } catch (err) {
