      ...
      {model && storage && !isExcluded && (
        <Section title="O aparelho esta trincado, quebrado ou com defeito?">
          <div className="flex gap-2">
            <Btn sel={hasDamage===false} onClick={() => setHasDamage(false)} className="flex-1" variant="success">Nao</Btn>
          ...
          {hasDamage === true && (
            <div className="mt-4 rounded-2xl p-4 text-center" style={{ backgroundColor: "var(--ti-error-light)", border: "1px solid var(--ti-error)" }}>
              <p className="text-[15px] font-semibold" style={{ color: "var(--ti-error)" }}>Infelizmente nao aceitamos aparelhos com tela trincada, quebrada ou com defeito na troca.</p>
            </div>
          )}
        </Section>
      )}
      ...
      {model && storage && !isExcluded && hasDamage === false && (
        <>
          <Section title="Saude da bateria">
            <div className="rounded-2xl p-4 space-y-3" style={{ backgroundColor: "var(--ti-card-bg)", border: "1px solid var(--ti-card-border)" }}>
              ...
              <div className="rounded-xl p-3" style={{ backgroundColor: "var(--ti-input-bg)", border: "1px solid var(--ti-card-border)" }}>
                <p className="text-[12px] font-semibold mb-1.5" style={{ color: "var(--ti-text)" }}>Como descobrir a saude da bateria:</p>
          ...
          {sideScratch !== null && <Section title="Descascado / Amassado"><div className="flex gap-2">
            {([["none","Nao"],["light","Leve"],["heavy","Forte"]] as const).map(([v,l]) => <Btn key={v} sel={peeling===v} onClick={() => setPeeling(v)} className="flex-1">{l}</Btn>)}
          </div></Section>}

          {peeling !== null && (
          <Section title="O aparelho ja teve alguma peca trocada?">
            <div className="grid grid-cols-1 gap-2">
              <Btn sel={partsReplaced==="no"} onClick={() => setPartsReplaced("no")} variant="success">Nao</Btn>
          ...
          {partsOk && (
          <Section title="Ainda esta na garantia Apple de 12 meses?"><div className="flex gap-2">
            <Btn sel={hasWarranty===false} onClick={() => { setHasWarranty(false); setWarrantyMonth(null); }} className="flex-1">Nao</Btn>
          ...
          <Section title="Ainda tem a caixa original do aparelho?"><div className="flex gap-2">
            ...
            <Btn sel={hasOriginalBox===false} onClick={() => setHasOriginalBox(false)} className="flex-1">Nao</Btn>
          </div></Section>)}
        </>
      )}
